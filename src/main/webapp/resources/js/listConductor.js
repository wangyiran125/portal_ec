...
$().ready( function() {
	...
	var $deleteBeaconButton = $("#deleteBeaconButton");
	var $deleteCoordinateButton = $("#deleteCoordinateButton");
	...
	$deleteDeviceButton.click( function() {
		...
		$.dialog({
			type: "warn",
			content: message("admin.dialog.deleteDeviceConfirm"),
			ok: message("admin.dialog.ok"),
			cancel: message("admin.dialog.cancel"),
			onOk: function() {
				$.ajax({
					url: "deleteConductors.jhtml",
					type: "POST",
					data: $checkedIds.serialize(),
					dataType: "json",
					cache: false,
					success: function(message) {
						...
						$deleteDeviceButton.addClass("disabled");
						$selectAll.prop("checked", false);
						$checkedIds.prop("checked", false);
					}
				});
			...
			onOk: function() {
				$.ajax({
					...
					success: function(message) {
						...
						$deleteBeaconButton.addClass("disabled");
						$selectAll.prop("checked", false);
						$checkedIds.prop("checked", false);
					}
				});
	...
	// 删除
	$deleteCoordinateButton.click(function() {
		var $this = $(this);
		if ($this.hasClass("disabled")) {
			return false;
		}
			...
			onOk: function() {
				$.ajax({
					...
					success: function(message) {
						...
						$deleteCoordinateButton.addClass("disabled");
						$selectAll.prop("checked", false);
						$checkedIds.prop("checked", false);
					}
				});
	...
	$selectAll.click( function() {
		...
		if ($this.prop("checked")) {
			...
			if ($enabledIds.filter(":checked").size() > 0) {
				...
				$deleteDeviceButton.removeClass("disabled");
				$deleteBeaconButton.removeClass("disabled");
				$deleteCoordinateButton.removeClass("disabled");
