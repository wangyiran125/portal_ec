package com.chinalbs.framework.dao.impl;
  ...
  public T find(ID id) {
    if (id != null) {
      if (LogUtil.isDebugEnabled(BaseDaoImpl.class)) {
        LogUtil.debug(BaseDaoImpl.class, "find", "Fetching entity from database with ID : %s", id);
      }
  ...
  private synchronized String getAlias(Selection<?> selection) {
    if (selection != null) {
      ...
      if (alias == null) {
        ...
        alias = "ECGeneratedAlias" + aliasCount++;
