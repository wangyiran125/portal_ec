...
import com.chinalbs.beans.Template;
import com.chinalbs.beans.Template.Type;
...
public class TemplateServiceImpl implements TemplateService, ServletContextAware {
	...
	public List<Template> getAll() {
		try {
			File xmlFile = new ClassPathResource(CommonAttributes.COMMON_CONFIG_XML_PATH).getFile();
			Document document = new SAXReader().read(xmlFile);
			List<Template> templates = new ArrayList<Template>();
			List<Element> elements = document.selectNodes("/ec/template");
	...
	public List<Template> getList(Type type) {
		if (type != null) {
			try {
				...
				Document document = new SAXReader().read(xmlFile);
				List<Template> templates = new ArrayList<Template>();
				List<Element> elements = document.selectNodes("/ec/template[@type='" + type + "']");
				for (Element element : elements) {
					Template template = getTemplate(element);
					templates.add(template);
				}
	...
	public Template get(String id) {
		try {
			File xmlFile = new ClassPathResource(CommonAttributes.COMMON_CONFIG_XML_PATH).getFile();
			Document document = new SAXReader().read(xmlFile);
			Element element = (Element) document.selectSingleNode("/ec/template[@id='" + id + "']");
			Template template = getTemplate(element);
			return template;
		} catch (Exception e) {
			e.printStackTrace();
