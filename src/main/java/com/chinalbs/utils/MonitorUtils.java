...
public class MonitorUtils {
  ...
  public static MonitorInfoBean getMonitorInfoBean(){
    ...
    infoBean.setPhymemoryUsage(get2Double((double)usedMemory/(double)totalMemorySize));
    infoBean.setMemoryUsage(get2Double((double)(totalMemory-freeMemory)/(double)maxMemory));
    return infoBean;
  }
    ...
    try {
      ...
      while ((line = input.readLine()) != null) {
        ...
        if (caption.equals("System Idle Process") || caption.equals("System")) {
          idletime += Long.valueOf(bytesubstring(line, kmtidx, rocidx - 1).trim()).longValue();
          idletime += Long.valueOf(bytesubstring(line, umtidx, wocidx - 1).trim()).longValue();
          continue;
        }

        kneltime += Long.valueOf(bytesubstring(line, kmtidx, rocidx-1).trim()).longValue();
        usertime += Long.valueOf(bytesubstring(line, umtidx, wocidx-1).trim()).longValue();
      }
      retn[0] = idletime;
...
//      System.out.println("内存使用率=" + monitorInfo.getMemoryUsage() * 100);  
//      System.out.println("物理内存使用率=" + monitorInfo.getPhymemoryUsage() * 100);  
//  }  
}
